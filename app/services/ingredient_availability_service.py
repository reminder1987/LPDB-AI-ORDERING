...
from app.models.ingredient_availability_db import IngredientAvailabilityDB
...
def _get_ingredient(db, ingredient_id: int, tenant_id: int):
    ingredient = db.scalar(
        select(IngredientDB)
        .options(selectinload(IngredientDB.category))
        .where(
    ...
    if ingredient is None:
        raise ValueError(f"Ingrediente no encontrado: {ingredient_id}")
    return ingredient
...
def _get_availability_record(db, ingredient_id: int, location_id: int):
    return db.scalar(
        select(IngredientAvailabilityDB).where(
            IngredientAvailabilityDB.ingredient_id == ingredient_id,
            IngredientAvailabilityDB.location_id == location_id,
        )
    )


def _is_blocking_ingredient(ingredient: IngredientDB) -> bool:
    return ingredient.category.name not in NON_BLOCKING_CATEGORIES


def _is_ingredient_available(db, ingredient_id: int, location_id: int) -> bool:
    record = _get_availability_record(db, ingredient_id, location_id)
    return True if record is None else record.available
...
def get_ingredient_availability(ingredient_id: int, location_id: int, tenant_id: int):
    db = SessionLocal()
    try:
        ...
        if record is None:
            return {
                ...
                "blocking": _is_blocking_ingredient(ingredient),
                "available": True,
...
def is_ingredient_available(ingredient_id: int, location_id: int, tenant_id: int) -> bool:
...
def mark_ingredient_unavailable(ingredient_id: int, location_id: int, tenant_id: int, reason: str | None = None):
...
def mark_ingredient_available(ingredient_id: int, location_id: int, tenant_id: int, reason: str | None = None):
    return set_ingredient_availability(
        ingredient_id=ingredient_id,
        location_id=location_id,
        tenant_id=tenant_id,
...
def set_external_ingredient_availability(
    ...
    if source != SOURCE_TOAST:
        raise ValueError(f"Fuente externa no soportada: {source}")
    return set_ingredient_availability(
        ingredient_id=ingredient_id,
        location_id=location_id,
        tenant_id=tenant_id,
...
def get_products_affected_by_ingredient(ingredient_id: int, tenant_id: int):
    db = SessionLocal()
    try:
        _get_ingredient(db, ingredient_id, tenant_id)
        products = db.scalars(
            select(ProductDB)
            .join(RecipeDB, RecipeDB.product_id == ProductDB.id)
            .where(
                ProductDB.tenant_id == tenant_id,
                RecipeDB.ingredients.any(ingredient_id=ingredient_id),
            )
            .order_by(ProductDB.name)
        ).all()
        return [{"product_id": p.id, "product": p.name} for p in products]
    finally:
        db.close()
...
def _recalculate_products_for_ingredient(db, ingredient_id: int, location_id: int, tenant_id: int):
    ...
    products = db.scalars(
        ...
        .options(
            ...
            .selectinload(RecipeDB.ingredients)
            .selectinload(RecipeDB.ingredients.property.mapper.class_.ingredient)
            .selectinload(IngredientDB.category),
        )
        .join(RecipeDB, RecipeDB.product_id == ProductDB.id)
        .where(
            ProductDB.tenant_id == tenant_id,
    ...
    for product in products:
        product_availability = db.scalar(
            select(ProductAvailabilityDB).where(
                ProductAvailabilityDB.product_id == product.id,
        ...
        if unavailable_ingredients:
            ...
            available = False
            reason = f"Ingrediente(s) agotado(s): {unavailable_names}"
        else:
            available = True
            reason = None
...
def recalculate_products_for_ingredient(ingredient_id: int, location_id: int, tenant_id: int):
    db = SessionLocal()
    try:
        _get_location(db, location_id, tenant_id)
...
def get_product_ingredient_blocking_reason(
    ...
    try:
        ...
        if not unavailable_ingredients:
            return None
        return "Ingrediente(s) agotado(s): " + ", ".join(
            ingredient.name for ingredient in unavailable_ingredients
        )
    finally:
        db.close()
